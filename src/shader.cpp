...
//  PuzzleGame
//
//  Created by Cesar Luis Valdez on 21/09/14.
...
void GLSLShader::AddAttribute(const string& attribute) {
    GLuint tmp = glGetAttribLocation(_program, attribute.c_str());
    assert(tmp != 0xFFFFFFFF);
    _attributeList[attribute] = tmp;
}
...
void GLSLShader::AddUniform(const string& uniform) {
    GLuint tmp = glGetUniformLocation(_program, uniform.c_str());
    assert(tmp != 0xFFFFFFFF);
    _uniformLocationList[uniform] = tmp;
}
